...
The bounded validation runner in `scripts/run_research_validation.py` executes the pending throughput search comparison on fresh measurements. It uses an eight candidate pool, a four candidate budget, a fresh backend default control, three random seeds for random and Bayesian search, an independent measured oracle, and three benchmark trials per candidate. The validation subset covers Qwen3 0.6B, Mistral 7B, Qwen3.8 27B, vLLM, SGLang, short, long prefill, and the pinned OASST1 root English set. OASST1 is split deterministically into a 48 prompt training set for search and a 16 prompt holdout set used only after selection. Evidence reuse and resume based failure memory are disabled for these comparisons so every search observation is a fresh server and request measurement.
