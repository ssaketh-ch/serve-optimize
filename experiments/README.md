...
The closure runner accepts `SERVE_OPTIMIZE_MODEL_SCOPE=wide` for the publication matrix. This scope covers Qwen 0.6B, Qwen 1.5B, DeepSeek distilled 1.5B, Granite 2B, Qwen 7B, Mistral 7B, Qwen 8B, Granite 8B, Qwen 3.5 9B, Qwen 3.6 27B, Qwen 3.8 27B, and Llama 3.1 8B when access is approved. For each model and backend it runs short, medium, and long prefill saturation, mixed, code generation, repeated prefix, permitted real chat, three balanced repetitions, and three GuideLLM cross checks. The full scope is 240 managed runs and 72 GuideLLM reports per hardware host. The Qwen3.8 model is evaluated through its text serving path; this does not claim multimodal performance. A model that cannot load on a backend remains an explicit failure or unsupported result rather than being silently removed from the matrix.
